...
import { getMyOrders, getMyActivity, getSessions, userNegotiate } from '../api';
  ...
  const handleNegotiationAction = async (action) => {
    try {
      ...
      if (action === 'counter') {
        ...
        payload.suggestedPrice = Number(counterPrice);
      }

      const res = await userNegotiate(selectedOrder._id, payload, token);
      
      // Update local state
      setOrders(prev => prev.map(o => o._id === selectedOrder._id ? res.data.order : o));
      setSelectedOrder(res.data.order);
      alert(action === 'accept' ? "Price Accepted! Order is now approved." : "Counter-offer sent to Admin.");
    } catch (err) {
      alert(err.response?.data?.msg || "Negotiation failed.");
    } finally {
      setIsNegotiating(false);
    }
  };
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-black/90 backdrop-blur-3xl">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-3xl bg-[#050507] border border-white/10 p-12 rounded-[3rem] shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-4 mb-8">
                ...
                {selectedOrder.negotiationStatus === 'awaiting_user' && (
                  <div className="p-6 rounded-3xl bg-indigo-600/10 border border-indigo-500/30 shadow-[0_0_25px_rgba(79,70,229,0.15)] animate-in slide-in-from-bottom-4 duration-500">
                    <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                      <span className="w-2 h-2 bg-indigo-500 rounded-full animate-ping" />
                      Action Required: Price Verification
                    </h4>
                    <p className="text-[11px] text-slate-300 mb-6 font-medium leading-relaxed">
                      The administrator has proposed a price of <span className="text-white font-black">${selectedOrder.totalAmount}.00</span> for this custom order. You can either accept this price or suggest a one-time counter-offer.
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <button 
                        ...
                        className="flex-1 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(16,185,129,0.2)]"
                      >
                        {isNegotiating ? "PROCESSING..." : "Accept Proposed Price"}
                      </button>
                      ...
                      <div className="flex-1 flex gap-2">
                        <input 
                          ...
                          className="w-1/2 bg-white/5 border border-white/10 rounded-2xl px-4 text-[10px] text-white outline-none focus:border-indigo-500"
                        />
                        <button 
                          disabled={isNegotiating || selectedOrder.hasCountered}
                          onClick={() => handleNegotiationAction('counter')}
                          className="flex-1 py-4 bg-white/5 border border-white/10 hover:bg-white hover:text-black text-slate-400 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all"
                        >
                          {selectedOrder.hasCountered ? "COUNTER_SENT" : "Counter Offer"}
                        </button>
                      </div>
                    </div>
                    {selectedOrder.hasCountered && (
                      <p className="text-[8px] text-amber-500/70 mt-3 italic font-bold text-center uppercase tracking-widest">
                        Note: You can only suggest a price once.
                      </p>
                    )}
                  </div>
                )}
                ...
                {selectedOrder.negotiationStatus === 'user_countered' && (
                  <div className="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/20">
                     ...
                     <p className="text-[10px] text-slate-300 mt-2 font-medium">
                       You have suggested a price of <span className="text-white font-black">${selectedOrder.userSuggestedPrice}.00</span>. Waiting for administrator to review and accept/reject.
                     </p>
                  </div>
                )}
              </div>
