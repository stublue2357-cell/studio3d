  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        // Registration with optional adminSecret
      ...
      const debugInfo = err.response?.data?.debug ? ` [DEBUG: ${err.response.data.debug}]` : "";
      setMsg(`${errorMsg}${debugInfo}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-6 overflow-hidden font-['Plus_Jakarta_Sans']">
      ...
      >
        ...
        {/* Left Side: Info */}
        <div className="md:w-5/12 p-12 border-r border-white/5 flex flex-col justify-between bg-gradient-to-br from-indigo-950/40 to-transparent">
          <div>
            <div className="text-2xl font-black italic mb-2 tracking-tighter text-white uppercase">STUDIO<span className="text-indigo-400">3D</span></div>
            <div className="h-px w-12 mb-8 bg-indigo-500" />
            <h2 className="text-5xl font-bold tracking-tighter leading-none mb-6 text-white uppercase italic">
              {isLogin ? 'AUTHENTICATE' : 'REGISTER'} <br /> 
              <span className="text-indigo-400">IDENTITY.</span>
            </h2>
            <p className="text-slate-400 text-[10px] uppercase tracking-[0.4em] font-bold leading-loose">
              {isLogin ? 'Secure node access protocol active.' : 'Join the elite design matrix.'}
            </p>
          </div>
        ...
        <div className="md:w-7/12 p-12 md:p-16 flex flex-col justify-center bg-black/40">
          <div className="mb-10 flex justify-between items-center border-b border-white/10 pb-8">
            <div>
              <h3 className="text-[11px] font-black tracking-[0.3em] text-white uppercase">
                {isLogin ? 'LOG INTO SYSTEM' : 'CREATE NEW NODE'}
              </h3>
            </div>
            <button 
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <input 
              type="email" 
              required 
              placeholder="NODE_EMAIL@STUDIO3D.AI" 
              className="w-full bg-white/[0.03] border border-white/10 px-8 py-5 rounded-2xl text-[11px] text-white outline-none focus:border-indigo-500/50 transition-all" 
            ...
            {!isLogin && (
              <div className="pt-2">
                <input 
                  ...
                  className="w-full bg-indigo-500/5 border border-indigo-500/10 px-8 py-5 rounded-2xl text-[11px] text-indigo-400 placeholder:text-indigo-900 outline-none focus:border-indigo-500/30 transition-all" 
            ...
            <button 
              disabled={isLoading} 
              className={`w-full py-6 rounded-2xl text-[11px] font-black uppercase tracking-[0.6em] transition-all relative overflow-hidden group ${
                isLoading 
                ? 'bg-slate-900 text-slate-700 cursor-not-allowed' 
                : 'bg-white text-black hover:bg-indigo-500 hover:text-white shadow-[0_20px_40px_rgba(79,70,229,0.2)]'
              }`}
            >
              <span className="relative z-10">{isLoading ? 'VERIFYING...' : (isLogin ? 'AUTHORIZE PORTAL' : 'INITIALIZE NODE')}</span>
            </button>
          </form>
          ...
          <AnimatePresence>
            {msg && (
              <motion.div 
                ...
                className={`mt-10 p-4 rounded-xl border text-center text-[10px] tracking-widest uppercase font-black ${
                  msg.includes('GRANTED') || msg.includes('SUCCESS') 
