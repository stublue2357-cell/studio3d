...
const AdminProducts = ({ isEmbedded }) => {
  ...
  const handlePurge = async (p) => {
    if (!window.confirm(`Are you sure you want to ${role === 'admin' ? 'request deletion for' : 'permanently delete'} this product?`)) return;
    
    try {
      const token = localStorage.getItem('token');
      if (role === 'admin') {
        const { requestDeleteProduct } = await import('../api');
        await requestDeleteProduct(p._id, token);
        alert("PURGE_REQUEST_SENT // AWAITING_OWNER_APPROVAL");
      } else {
        await deleteProduct(p._id, token);
        alert("PRODUCT_PURGED // MAIN_FRAME_UPDATED");
      }
      fetchProducts();
    } catch (err) {
      alert("PURGE_FAILED // ACCESS_DENIED");
    }
  };
