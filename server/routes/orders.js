  ...
  try {
    ...
    if (global.isSimulationMode) {
        ...
        if (totalAmount) {
            ...
            order.negotiationStatus = 'awaiting_user';
        }
...
// 6. USER NEGOTIATION ACTION (Accept or Counter)
router.patch('/negotiate/:id', authMiddleware, async (req, res) => {
    try {
        const { action, suggestedPrice } = req.body;
        ...
        if (global.isSimulationMode) {
            ...
            if (action === 'accept') {
                ...
                if (order.hasCountered) return res.status(400).json({ msg: "COUNTER_LIMIT_REACHED // ONLY_ONCE" });
                order.negotiationStatus = 'user_countered';
                order.userSuggestedPrice = Number(suggestedPrice);
                order.hasCountered = true;
            }
            return res.json({ msg: "NEGOTIATION_UPDATED // MOCK_SUCCESS", order });
        }
        ...
        await order.save();
        res.json({ msg: "NEGOTIATION_SENT // SUCCESS", order });
    } catch (err) {
        res.status(500).json({ msg: "NEGOTIATION_ERROR // TRANSMISSION_FAILED" });
    }
});

// 7. ADMIN NEGOTIATION RESPONSE (Accept Counter or Reject)
router.patch('/negotiate/admin/:id', authMiddleware, anyAdminLevel, async (req, res) => {
    try {
        ...
        if (global.isSimulationMode) {
            ...
            if (action === 'accept') {
                order.totalAmount = order.userSuggestedPrice;
                order.negotiationStatus = 'accepted';
                order.status = 'Approved';
            } else if (action === 'reject') {
                order.negotiationStatus = 'rejected';
                order.status = 'Rejected';
            }
            return res.json({ msg: "NEGOTIATION_FINALIZED // MOCK_SUCCESS", order });
        }
        ...
        await order.save();
        res.json({ msg: "NEGOTIATION_FINALIZED // SUCCESS", order });
    } catch (err) {
        res.status(500).json({ msg: "FINALIZATION_ERROR // TRANSMISSION_FAILED" });
    }
});
