...
import { getAllOrdersAdmin, updateOrderStatus, adminNegotiate } from '../api';
...
const AdminOrders = ({ isEmbedded }) => {
  ...
  const handleNegotiationResponse = async (id, action) => {
    try {
      const token = localStorage.getItem('token');
      const res = await adminNegotiate(id, { action }, token);
      alert(action === 'accept' ? "Counter-offer accepted! Price updated." : "Counter-offer rejected. Order cancelled.");
      setSelectedOrder(res.data.order);
      fetchOrders();
    } catch (err) {
      alert(err.response?.data?.msg || "Response failed.");
    }
  };
        ...
        {isModalOpen && selectedOrder && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-black/95 backdrop-blur-2xl">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-4xl bg-[#050507] border border-white/10 p-12 rounded-[3.5rem] shadow-2xl relative overflow-hidden">
              ...
              {/* --- NEGOTIATION RESPONSE MODULE --- */}
              {selectedOrder.negotiationStatus === 'user_countered' && (
                <div className="mt-12 p-8 bg-indigo-600/10 border border-indigo-500/30 rounded-[2.5rem] relative z-10 shadow-[0_0_40px_rgba(79,70,229,0.1)] animate-in zoom-in-95 duration-500">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <span className="text-[9px] font-black text-white bg-indigo-500/20 px-4 py-1 rounded-full border border-indigo-500/30">
                      USER_SUGGESTED: ${selectedOrder.userSuggestedPrice}.00
                    </span>
                  </div>
                  
                  <p className="text-[11px] text-slate-300 mb-8 font-medium leading-relaxed">
                    The user has declined your proposed price of <span className="text-white font-black">${selectedOrder.totalAmount}.00</span> and suggested <span className="text-indigo-400 font-black">${selectedOrder.userSuggestedPrice}.00</span> instead. <br />
                    <span className="text-slate-500 text-[9px] italic mt-2 block italic">Accepting this will update the total amount and approve the design protocol.</span>
                  </p>

                  <div className="flex gap-4">
                    <button 
                      onClick={() => handleNegotiationResponse(selectedOrder._id, 'accept')}
                      className="flex-1 py-5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-[0_0_25px_rgba(16,185,129,0.25)]"
                    >
                      Accept Counter-Price
                    </button>
                    <button 
                      onClick={() => handleNegotiationResponse(selectedOrder._id, 'reject')}
                      className="flex-1 py-5 bg-rose-600/10 border border-rose-500/30 hover:bg-rose-600 text-rose-500 hover:text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all"
                    >
                      Reject Offer & Order
                    </button>
                  </div>
              ...
              {selectedOrder.negotiationStatus === 'accepted' && (
                <div className="mt-8 p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl relative z-10 flex items-center justify-center gap-3">
                  <span className="text-emerald-500 text-lg">✓</span>
                  <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Negotiation Finalized // Price Locked</span>
                </div>
              )}
